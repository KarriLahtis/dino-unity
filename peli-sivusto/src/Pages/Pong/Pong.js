  ...
  const Ball = {
    new: function (incrementedSpeed) {
      return {
        width: 18,
        height: 18,
        x: (Pong.canvas.width / 2) - 9,
        y: (Pong.canvas.height / 2) - 9,
  ...
  const Ai = {
    new: function (side) {
      return {
        width: 18,
        height: 180,
        x: side === 'left' ? 150 : Pong.canvas.width - 150,
        y: (Pong.canvas.height / 2) - 35,
        score: 0,
        move: DIRECTION.IDLE,
        speed: 8,
      };
  ...
  const Game = {
    ...
    update: function () {
      if (!this.over) {
        ...
        // Move ball in the intended direction based on moveY and moveX values
      ...
      // Check if the ai/AI has won the round.
    ...
    // Reset the ball location, the player's turns, and set a delay before the next round begins.
